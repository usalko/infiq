...
class Message:
    ...
    def parse(message: bytes) -> Any:
        if len(message) < 16:
            raise EtError('Not valid message, length less then 16')
        ...
        raise EtError('Not valid message, unknown message type')
    ...
    def as_int(self) -> int:
        ...
        raise EtError(f'Sorry, but I can\'t read not number message. Message type is {self.message_type}')
